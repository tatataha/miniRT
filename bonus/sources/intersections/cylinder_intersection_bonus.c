...
bool	check_walls(t_cylinder *cy, t_hit *inter, double t)
{
	...
	m = vector_dot(inter->ray.direction, cy->normal) * t + vector_dot(co,
			cy->normal);
...
double	verify_intersections(t_cylinder *cy, t_ray *ray, t_equation *eq,
		t_hit *hit_info)
{
	double	t3;
	double	t4;
...
bool	cylinder_intersect(t_cylinder *cy, t_ray *ray, t_hit *hit_info)
{
	...
	eq.b = 2 * (vector_dot(ray->direction, cy_to_ray)
			- (vector_dot(ray->direction, cy->normal) * vector_dot(cy_to_ray,
					cy->normal)));
	eq.c = vector_dot(cy_to_ray, cy_to_ray) - pow(vector_dot(cy_to_ray,
				cy->normal), 2) - pow(cy->radius, 2);
	solve(&eq);
	if (eq.t1 <= 0 && eq.t2 <= 0)
		return (false);
